...
└──────────┬─────────┘    └──────────┬─────────┘
│ Bulk load               │ Every 12 hrs
▼                         ▼
┌──────────────────┐    ┌──────────────────┐
│   CloudShell     │    │  AWS Lambda      │
│   (Python)       │    │  (Python 3.12)   │
└────────┬─────────┘    └────────┬─────────┘
│                       │
▼                       ▼
...
│           Amazon S3 Data Lake              │
│ ┌─────────────────┐ ┌────────────────────┐ │
│ │ Historical (CSV)│ │ Live (JSON)        │ │
│ │ Partitioned by  │ │ Partitioned by     │ │
│ │ year=YYYY/      │ │ year/month/day/hr/ │ │
│ └─────────────────┘ └────────────────────┘ │
└─────────────────┬──────────────────────────┘
│
▼
...
│  ├── flights (external table, partitioned) │
│  ├── flights_clean (view: cleaning + FE)   │
│  ├── route_performance (analytics view)    │
│  ├── airline_performance (analytics view)  │
│  ├── hourly_patterns (analytics view)      │
│  └── covid_impact (analytics view)         │
└─────────────────┬──────────────────────────┘
│
┌─────────────┼──────────────┐
▼             ▼              ▼
┌──────────┐  ┌───────────┐  ┌────────────┐
│ ML Model │  │ Streamlit │  │ REST API   │
│ (RF AUC  │  │ Dashboard │  │ (Future)   │
│  0.67)   │  │           │  │            │
└──────────┘  └───────────┘  └────────────┘
### AWS Services Used
